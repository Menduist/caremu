 ...
 * Uniq
*/
void vector_uniq(struct vector *vect) {
	void *last = NULL;
	int i = 0, j = 0;
	while (i < vect->current) {
		if (vect->data[i] != vect->data[j]) vect->data[++j] = vect->data[i];
		i++;
	}
	vect->current = j;
}
...
void vector_extends_backk(struct vector *this, struct vector *next) {
	while (this->limit < this->current + next->current) {
		if (vector_realloc(this))
			return;
	}
	memmove(this->data + this->current, next->data, next->current * sizeof(void *));
}
